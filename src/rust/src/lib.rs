...
#[extendr]
fn gnu_parallel_run_task_group(task_group: Robj, ssh_login_file: Robj, pipeline_dir: Robj) {
    let ssh_login_file_str = ssh_login_file
        .as_str()
        .unwrap_or_else(|| panic!("ssh_login_file parameter must be a string"))
        .to_string();

    let pipeline_dir_str = pipeline_dir
        .as_str()
        .unwrap_or_else(|| panic!("pipeline_dir parameter must be a string"))
        .to_string();
